...
def login():
    if request.method == "POST":
        ...
        if usuario.militar and not usuario.militar.ativo:
            flash("Seu acesso está bloqueado porque seu cadastro está inativo.", "danger")
            return redirect(url_for("auth.login"))

        if usuario.militar and militar_bloqueado_por_posto(usuario):
            flash("Militares com posto/graduação Cabo ou Soldado não possuem acesso ao sistema.", "danger")
            return redirect(url_for("auth.login"))

        login_user(usuario)

        # Conta ADMIN é conta técnica/administrativa e pode não possuir identidade militar.
        # Portanto, não deve ficar presa no fluxo de primeiro acesso por ausência de identidade.
...
def esqueci_senha():
    if request.method == "POST":
        ...
        flash("Se os dados estiverem corretos, a solicitação será enviada ao administrador.", "info")
        return redirect(url_for("auth.login"))
...
def admin_solicitacoes_senha():
    ...
    return render_template("auth/admin_solicitacoes_senha.html", solicitacoes=solicitacoes)
...
def aprovar_solicitacao_senha(solicitacao_id):
    ...
    flash("Solicitação aprovada. A senha foi resetada para a identidade militar.", "success")
    return redirect(url_for("auth.admin_solicitacoes_senha"))
